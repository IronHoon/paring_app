...
function FollowersPage(props) {
  ...
  const getFollowersData = async () => {
    setLoadingFollowers(true);
    try {
      const [data] = await getMyFollowers(pageFollowers.current);
      ...
      if (!(pageFollowers.current > lastPageFollowers.current)) {
        pageFollowers.current = pageFollowers.current + 1;
  ...
  const getFollowingsData = async () => {
    setLoadingFollowings(true);
    try {
      const [data] = await getMyFollowings(pageFollowings.current);
      const _data = data?.data;
      ...
      if (!(pageFollowings.current > lastPageFollowings.current)) {
        pageFollowings.current = pageFollowings.current + 1;
...
FollowersPage = withContext(FollowersPage);
